...
def head(uri, timeout=20, headers=None):
    """
    Execute an HTTP GET query on `uri`, and return the headers.  `timeout` is
    an optional argument, which represents how much time we should wait before
    throwing a timeout exception. It defualts to 20, but can be set to higher
    values if you are communicating with a slow web application.
    """
    if not uri.startswith('http'):
        uri = "http://" + uri
    u = get_urllib_object(uri, timeout, headers)
...
def post(uri, query, limit_bytes=None):
    """
    Execute an HTTP POST query. `uri` is the target URI, and `query` is the
    POST data. `headers` is a dict of HTTP headers to send with the request.
    If `limit_bytes` is provided, only read that many bytes from the URL. This
    may be a good idea when reading from unknown sites, to prevent excessively
    large files from being downloaded.
    """
...
def get_urllib_object(uri, timeout, headers=None):
    """
    Return a urllib2 object for `uri` and `timeout` and `headers`. This is
    ...
    they need a urllib2 object to execute .read() on. For more information,
    refer to the urllib2 documentation.
    """
    try:
...
def urlencode(data):
    """
    Identical to urllib.urlencode. Use this if you already importing web in
