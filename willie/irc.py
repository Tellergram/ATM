...
class Bot(asynchat.async_chat):
    ...
    def msg(self, recipient, text, max_messages=1):
        ...
        if max_messages > 1 and len(encoded_text) > max_text_length:
            last_space = -1
            for x in ['. "','! "','? "']:
                pos = encoded_text.rfind(x.encode('utf-8'), 0, max_text_length)
                if pos != -1:
                    last_space = max(last_space,pos+1)
            if last_space == -1:    
                for x in ['. ','! ','? ']:
                    pos = encoded_text.rfind(x.encode('utf-8'), 0, max_text_length)
                    if pos != -1:
        ...
        text = encoded_text.decode('utf-8').strip()
        try:
            ...
            if recipient_id not in self.stack:
                ...
                if elapsed < 3:
                    penalty = float(max(0, len(text) - 50)) / 700
                    wait = 0.2 + penalty
                    if elapsed < wait:
